...
module Noteable
  ...
  class_methods do
    ...
    # `Noteable` class names that support forwarding individual notes.
    def email_forwardable_types
      %w(Issue)
    end
  end
  ...
  def supports_forwarding_notes_by_email?
    self.class.email_forwardable_types.include?(base_class_name)
  end
  ...
  def forwardable_note_email_address(author)
    return unless supports_forwarding_notes_by_email?
