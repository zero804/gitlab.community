  ...
  context 'when authenticated as the namespace owner' do
    ...
    context 'when querying multiple namespaces' do
      ...
      it 'responds with the expected list of compliance frameworks' do
        ...
        expect(graphql_data_at(:b, :complianceFrameworks, :nodes).map { |f| f['name'] }).to contain_exactly('GDPR', 'SOX')
        expect(graphql_data_at(:c, :complianceFrameworks, :nodes).map { |f| f['name'] }).to contain_exactly('SOX')
      end
    end
