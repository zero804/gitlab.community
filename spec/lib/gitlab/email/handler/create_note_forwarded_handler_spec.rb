...
RSpec.describe Gitlab::Email::Handler::CreateNoteForwardedHandler do
  ...
  let(:email_raw) { email_fixture('emails/valid_note_forwarded.eml') }
  ...
  context 'when the recipient address does not include a mail key' do
    ...
    let(:email_raw) { fixture_file('emails/valid_note_forwarded.eml').gsub(mail_key, '') }
