...
RSpec.describe Noteable do
  ...
  describe '.email_forwardable_types' do
    it 'exposes the email creatable types' do
      expect(described_class.email_forwardable_types).to include('Issue')
    end
  end
  ...
  describe '#forwardable_note_email_address' do
    ...
    context 'incoming email enabled' do
      ...
      it 'returns the address to forward a note' do
        address = "p+#{project.full_path_slug}-#{project.project_id}-#{user.incoming_email_token}-issue-#{issue.iid}@gl.ab"

        expect(issue.forwardable_note_email_address(user)).to eq(address)
      end

      it 'returns nil for unsupported types' do
        expect(snippet.forwardable_note_email_address(user)).to be_nil
      end
    end
    ...
    context 'incoming email disabled' do
      ...
      it 'returns nil' do
        expect(issue.forwardable_note_email_address(user)).to be_nil
      end
    end
  end
end
