  ...
  describe '#execute' do
    ...
    context 'when the merge_request_reviewers flag is enabled' do
      ...
      context 'assign_reviewer command with multiple assignees' do
        ...
        it 'does not assign the first user if they are unpermitted' do
          project.add_guest(developer2)

          content = "/assign_reviewer @#{developer2.username} @#{developer3.username}"
          _, _, message = service.execute(content, merge_request)

          expect(message).to eq("#{developer2.to_reference} doesn't have access to review this merge request.")
        end
      end
      ...
      context 'assign_reviewer where the only requested reviewer is invalid' do
        let(:content) { "/assign_reviewer @#{developer3.username}" }
        let(:issuable) { merge_request }

        it 'throws an error' do
          _, _, message = service.execute(content, merge_request)

          expect(message).to eq("#{developer3.to_reference} doesn't have access to review this merge request.")
        end
      end
